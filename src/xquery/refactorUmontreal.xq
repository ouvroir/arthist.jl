...
let $harRecords := $records//oai:record[oai:metadata/*:thesis/*:degree[*:level = 'Doctorat'][*:discipline="Histoire de l'art"]]
let $content := <oai:OAI-PMH>{
  $records[1]/oai:OAI-PMH/oai:responseDate,
  <ListRecords>{$harRecords}</ListRecords>
}</oai:OAI-PMH>

return file:write("src/data/" || "etdms-umontreal.xml", $content)